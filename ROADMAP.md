...
| ComStar DM — store and deliver | 🔬 | A minimal live-delivery prototype now exists on the branch: the all-roster inquiry submenu can open a `cmd 36` compose prompt, the client replies with `cmd 21`, and the server delivers the message immediately to an online target as a private `Cmd3` line while acknowledging the sender. Remaining gaps: offline persistence, unread delivery, exact original UI semantics, and confirmation of whether the real client used a distinct inbound DM command instead of `Cmd3`. |
| All-roster query | 🔬 | Global presence query: returns every online player's ComStar ID, handle, current sector, and location; triggered via KP5. Current RE confirms the `Cmd9 -> 0x3fd` roster popup is room-local occupant selection, not this feature, and the room menu `Cmd7(listId=3)` `selection=1` is the actual `All` request. The current branch prototype now follows the stronger RE path: `Cmd7(listId=3, selection=1)` sends `Cmd48_KeyedTripleStringList` (`0x51`) with live world sessions as rows, row picks (`Cmd7(listId, item_id + 1)`) open the inquiry submenu at `listId=1000`, and that submenu can branch to either a ComStar compose prompt (`cmd 36` / `cmd 21`) or `Cmd14_PersonnelRecord`. The older `Cmd45`/`Cmd58` family still looks like a separate scroll-list shell/list-id helper rather than the minimal KP5 reply. Remaining gap: confirm this flow against the real GUI client, especially whether KP5 truly reuses the local `1000` submenu unchanged. |

**Verification:** Local direct world-session smoke now covers `Cmd48` all-roster listing, row-pick inquiry submenu, `cmd 36` compose prompt, `cmd 21` text submit, immediate ComStar delivery to the selected online target, and sender acknowledgment.
