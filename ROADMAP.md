...
| RE world scene-action family | 🔧 | World opcode `17` is now partially decoded (RESEARCH.md §19.6.0a): subtype `1/2` = base agreement offer/review pair, `3` = duel, `4` = membership bid, `5/6/7` = subcontract offer/review/terms. The shared panel-mode byte now reads as edit/details/counter-offer/review (`0/1/2/3`). Remaining work is a later live capture to map concrete scene `cmd5 actionId` values to opcode `17` subtypes and recover the exact in-game noun for subtype `1/2`. |
...
| Task | Status | Notes |
|---|---|---|
| RE server→client combat bootstrap / position sync | 🔧 | v1.23 partial decode in RESEARCH.md §19.6.1: `Cmd72` seeds local combat actor state and now has a traced builder checklist covering scenario/title, terrain point lists, identity strings, initial coordinates, mech id, and initial local damage-state blocks; `Cmd64` adds remote actors/bots; `Cmd65` is the server→client combat position/motion update. Cmd65's four trailing motion fields now map to facing, throttle velocity, leg velocity, and forward/speed magnitude; live capture is still needed for signed direction conventions and exact `Cmd72` identity/status labels. **`Cmd62` (wire `0x5F`) closes the bootstrap loop** — it clears `DAT_0047ef60` bit `0x20`, enabling SPACEBAR weapon fire. Must be sent after all Cmd64 + initial Cmd65 packets. Full bootstrap sequence and `DAT_0047ef60` flag map documented in RESEARCH.md §19.9. SPACEBAR confirmed firing in live test. |
...
| RE TIC system | 🔧 | Three Targeting Interlock Circuits (A/B/C): v1.23 RE shows TIC membership toggles are local arrays (`DAT_004f2128`, `DAT_004f2150`, `DAT_004f2178`) and TIC group fire calls a local effect path, with no separate network sender found. Dynamic capture still needed to prove whether `cmd 12/action 0` means selected weapon, selected TIC group, or all queued fire. |
...
| RE jump jets | 🔧 | Fire command **decoded** (§19.3): client sends ESC+'!'+0x2D+0x25+CRC (cmd=12, action=4) via `Combat_SendCmd12Action_v123('\x04')`; landing/touchdown sends `cmd 12/action 6`. Fuel depletion, regeneration, Z-altitude tracking, and server→client feedback still 🔬. |
...
| Scripted dummy opponent | 🔧 | Basic bot implemented: spawns as "Opponent/Opponent" (slot 1 via Cmd64), is visible at `x=0,z=300000` (~300 m from origin, clear of center building), targetable with ENTER (after pressing `=` twice for ally mode), position refreshed every 1 s via Cmd65. SPACEBAR confirmed firing against bot in live test. Remaining: bot does not fire back; no damage dealt to player; no match end condition. See RESEARCH.md §19.9 for full bootstrap sequence and coordinate encoding notes. |
...
8. **Movement packets** — ✅ DECODED (RESEARCH.md §19.2): Cmd8 (coasting) and Cmd9 (moving) client→server formats fully mapped; bias/divisor constants confirmed. Server→client position (Cmd65) implemented and confirmed in live combat test.
9. **Weapon fire / damage packets** — ✅ DECODED (RESEARCH.md §19.3 / §19.9): `cmd 12/action 0` is the weapon-fire request; SPACEBAR confirmed firing in live test with dummy bot. Server `Cmd62` (wire `0x5F`) unblocks the fire gate by clearing `DAT_0047ef60` bit `0x20`. Damage model (Cmd66/Cmd67) partially decoded; round-trip hit confirmation still 🔬.
