...
| Scripted dummy opponent | 🔧 | Basic bot implemented: spawns as "Opponent/Opponent" (slot 1 via Cmd64), is visible at `x=0,z=300000` (~300 m from origin, clear of center building), targetable with ENTER (after pressing `=` twice for ally mode), position refreshed every 1 s via Cmd65. SPACEBAR confirmed firing against bot in live test. **Bot now fires back every 3 s via Cmd67 (issue #81, ✅).** Remaining: no match end condition when bot HP=0 or player HP=0; per-mech `internalStateBytes` still generic (issue #80). See RESEARCH.md §19.9 for full bootstrap sequence and coordinate encoding notes. |
