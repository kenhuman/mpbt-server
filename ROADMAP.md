...
| RE jump jets | 🔧 | Fire command **decoded** (§19.3): client sends ESC+'!'+0x2D+0x25+CRC (cmd=12, action=4) via `Combat_SendCmd12Action_v123('\x04')`; landing/touchdown sends `cmd 12/action 6`. **Prototype implementation now in place:** timed ascent/descent Cmd65 arc, action-6 forced landing, fuel drain while airborne, per-frame + passive grounded fuel regeneration. Remaining 🔬: exact v1.23 fuel constants, regeneration rates, and authoritative altitude semantics from live capture. |
