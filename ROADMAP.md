...
| RE movement protocol | 🔬 | Client → server movement commands; server → client position/environment updates. RazorWing's Type P/D/S notes were revalidated against our binary as combat-mode leads, not this M5 world-navigation path. The map UI now has a concrete travel-reply primitive: server cmd 40/43 opens Inner Sphere/Solaris map UI, and user confirmation sends client `cmd 10` with `type1 contextId` plus `type4 selectedRoomId+1` (`0` = cancel). Current branch prototype exposes this via `/map` or `/travel` world-chat debug trigger and a normal `Cmd4` scene option (`Travel` → client `cmd 5` action `4`), then uses the returned `cmd 10` to update server-side `map_room_<roomId>` presence grouping and reseed occupants with `Cmd6 -> Cmd4 -> Cmd10 -> Cmd3 -> Cmd5`. |
