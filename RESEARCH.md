...
| 9 | `0x2a` | `FUN_0040C310` | `Cmd9_RoomPlayerList` | Reads sentinel byte; if `0x01`: reads 1-byte count, then for each player reads a `Frame_ReadArg` string entry into `DAT_004de000` (40-byte slots). `FUN_0042daa0` later formats each slot as `"%d. %s"` for the roster window, so the payload is a plain string list, not a richer binary player struct. Calls `FUN_0042da40`, which opens a shared `0x3fd` prompt via `FUN_00413800(0x3fd, FUN_00405840(5), NULL)` and installs `FUN_00413a60` as the Enter-key handler; the `0x3fd` branch copies the chosen text into `DAT_004ddfd0`, invokes `FUN_0042daa0`, and marks the UI state at `DAT_004ddfc0+0x44 = 0xc`. From there `FUN_0040ca70` handles the roster popup: pressing Enter on an occupant with menu-id `0` opens `FUN_00412980`, a second-stage `MPBT.MSG` menu titled `0x90` ("Personal inquiry on:") with options `0x91` ("Send a ComStar message") and `0x92` ("Access personnel data"). That menu stores the selected occupant id and installs callbacks `FUN_00412190` / `FUN_004122d0`, which emit either `Cmd7(menuId, selection)` packets via `FUN_0040d2f0` or `0x1d, 0x02, <targetId>, <selection>` packets via `FUN_00410cc0`. Separately, the numbered selection callback `FUN_0042dbf0` emits client packet bytes `0x09, 0x01, <DAT_004ddfd0 string>, <selected-index byte>` via `FUN_0040d400`, before closing the popup. This path is the room-occupant inquiry UI, not the unresolved global KP5 all-roster query. Inference from the full handler chain: earlier use of `Cmd9` as a passive world-entry roster packet is now very doubtful and needs real-client validation before server-side reuse. |
...
- Additional issue #26 boundary from this pass: `FUN_00416db0` still has only three confirmed callers (`FUN_00412190` inquiry submenu, `FUN_00415f50` reply, and `Cmd37_OpenCompose`), and its buttons/messages remain ComStar-specific even when invoked as `FUN_00416db0(0, NULL)`. The local resource file `MPBT.MSG` still contains the early strings `Character Generation`, `Enter your character's name`, `Choose your allegiance:`, and `Enter choice:`, but this pass did not find live `FUN_00405840(4/5/6/7)` call sites or referenced message-id tables for them. Strongest current inference: the original online callsign-entry path remains unresolved, and the current `Cmd36` / `Cmd37` work should not be treated as proof of the real first-login name dialog.
