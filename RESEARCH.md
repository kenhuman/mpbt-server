...
| 9 | `0x2a` | `FUN_0040C310` | `Cmd9_RoomPlayerList` | Reads sentinel byte; if `0x01`: reads 1-byte count, then for each player reads a `Frame_ReadArg` string entry into `DAT_004de000` (40-byte slots). `FUN_0042daa0` later formats each slot as `"%d. %s"` for the roster window, so the payload is a plain string list, not a richer binary player struct. Calls `FUN_0042da40`, which in turn opens a shared text/menu prompt via `FUN_00413800(0x3fd, FUN_00405840(5), NULL)` and installs `FUN_00413a60` as the Enter-key handler; when the user confirms a selection, the `0x3fd` branch copies the chosen text into `DAT_004ddfd0`, invokes `FUN_0042daa0`, and marks the UI state at `DAT_004ddfc0+0x44 = 0xc`. The numbered selection callback `FUN_0042dbf0` then validates the chosen digit and emits client packet bytes `0x09, 0x01, <DAT_004ddfd0 string>, <selected-index byte>` via `FUN_0040d400`, before closing the popup. This path is the room-occupant inquiry UI, not the unresolved global KP5 all-roster query. Inference from the handler shape: earlier use of `Cmd9` as a passive world-entry roster packet is now doubtful and needs real-client validation. |
